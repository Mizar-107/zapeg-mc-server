...
- Fixed `/zapeg-lore` rejecting every OP and RCON command with "Heraldor does not accept this command source": the source gate reflected on CommandSourceStack's private field and called mapped-name methods on raw objects, which throw on the production server's SRG names — all branches fell through to reject. The gate now uses only exposed surfaces (`hasPermission`, `.player`, `.textName`) and logs rejected sources to the KubeJS log. Boundary change (accepted): a command block impersonating an OP via `execute as` is no longer distinguished — only the admin can program command blocks here.
- Fixed the ZapeG menu logo never appearing: Ice and Fire's client config defaults to a custom dragon main menu that replaces the title screen outright, so PackMenu (and our logo) never rendered. The patch now ships `config/iceandfire-client.toml` with `"Custom main menu" = false` (builder validates and requires it in the zip).
- Citizens field-failure hardening (brain 0.4.2, from the first live job report — sugar-cane collection stalled asking for a hoe, re-asked after "elinle kaz", citizen stuck in water): the harvest-policy fault net now covers hoes/çapa/orak and the whole surface-plant family (sugar cane, bamboo, crops — TR spellings with İ/ı classes), so both the first ask AND the post-answer re-ask are refused server-side; the planner prompt adds "respect a no-tool answer, never repeat that tool request" and "if stuck in water, move to dry land first"; and a new `harvest_cane` staged template (default count 8, quantity-gated on real sugar-cane gains, bank-walking guidance baked in) catches "şeker kamışı topla" phrasings deterministically. Brain tests 139 → 141.
- Added `scripts/citizens-debug-bundle.sh`: one host command produces one shareable file — compose state, mod jars, filtered mc + brain logs, rcon citizen list/brain-status/jobs, a read-only SQLite job dump, heraldor container state and the KubeJS `[zapeg-lore]` log — so debugging no longer needs interactive host access.
- Added "Zape Kayıtları III — Meclis Tutanakları" to `/zapeg-kitap`: council-minute humor covering every player including the two without accounts yet (the android inquiry into Enes Ö., Yusuf S.'s mangal authority, the fart-joke ban, the free-labor plaque, the iron petition, the chest-immunity hearings, the zartinium permit).
- Added the "ZapeG — Heraldor" quest tab (owner-approved foreshadowing: the NAME is now visible; mechanics stay secret): one octagon chapter, black-candle icon, a single "❞" placeholder quest whose only task is the hidden `zapeg:dokuzuncu` advancement — it completes for a player the night an OP grants it.
- Added the default HD look (owner request): the server now pushes **Faithful 32x "June 2026"** as its resource pack — HD but strictly vanilla-faithful, every block/item recognizable, ~11 MB one-time download from the official Modrinth CDN (sha1-pinned, credit to faithfulpack.net in the join prompt). Players get the standard accept prompt on join and may decline; mod textures deliberately stay 16x. Zero client-patch/lock impact; removal is deleting four compose lines.
- Fixed `/zapeg-unvan` and `/zapeg-kitap` never registering on the live server (while same-wave `/zapeg-cark` did): both hung their choice literals UNDER the player-argument node — the one Brigadier shape no other working ZapeG command uses. Both trees now use the servant-proven literal-first shape (`ver <seçim> <oyuncu>`), the unvan advancement hook registers defensively (a missing `PlayerEvents.advancement` binding now disables automation with a log line instead of killing the whole file), and every zapeg script prints a version-stamped `[zapeg] ... loaded` canary at load so the debug bundle shows exactly which scripts parsed and which gate version is live.
- Added `scripts/citizens-debug-bundle.ps1` — the live host is Windows with WSL docker, where the bash bundle can't run reliably: the PowerShell twin picks native docker or `wsl -e docker` automatically, includes a `git status -sb` deploy-lag detector (behind-origin count), and greps the `[zapeg` canaries. The `.sh` keeps working on real-Linux hosts (fragile `/dev/tty` tail removed).
- ROOT CAUSE FOUND AND VERIFIED on a new local test harness (bare Forge 47.4.10 + the live pack's exact kubejs/rhino/architectury jars, driven end-to-end on the maintainer's machine): this Rhino does not block-scope `const`/`let` inside loop bodies — the second iteration throws `redeclaration of var` at runtime, killing that commandRegistry handler AND every later script's handler in alphabetical order. That, not the Brigadier tree shape, is why `/zapeg-unvan` and `/zapeg-kitap` never registered while `/zapeg-cark` (earlier alphabetically, no loop-const) did. The same landmine sat unexploded in servant spawn placement, `/zapeg-unvan liste`, and quest_validation's personal-rewards tick loop — all converted to `var` (with the attach-helper pattern where a callback must capture the loop value, since a shared `var` binding would leak the last value into every callback). Registration now PROVEN before shipping: the harness selftest reports all four commands registered with all expected children and all console executions passing, including `story status` through gate v3. Harness selftest versioned at `tools/kubejs-selftest.js`.
- Player lore round 1 (owner-supplied): welcome-line pools, Muhtar memory cards and Heraldor dossier whispers updated — Salih's fart jokes + Comolokko, kralxlarge's zartinium/zurtinium grind, MertOnal the village's only real builder (iron/coal deficit), Thekingim's chest-ore "borrowing" + per-patch client relearning, Darkfire6161 (Yunus, Trabzon, everyone's favorite, Emir's special treatment), eminomi12 works for free, Yusuf the mangal chief, and the "second Enes may be an android" village rumor.
...
- Added Numen AI 0.1.1 (CurseForge file `8551640`) and the reviewed ZapeG Citizens 0.4.0 Forge jar to both the server and client inventory locks.
...
- Added `/citizen spawn <name> <onlinePlayer>`, assignment-aware chat routing, all 32 server-executable Numen tools, per-citizen/per-actor dialogue memory and stop/remove safety. Citizens can use Numen's movement, mining, building, crafting, storage, item-transfer, interaction and combat capabilities; only its two client-only tools remain unavailable to the server controller.
- Upgraded physical work to protocol-3 durable jobs: SQLite and Minecraft `SavedData` persist goals, actor anchors, plans, checkpoints, pending actions, evidence, budgets and cancellation state across restarts. `@Name status|stop|answer`, `/citizen status|jobs|resume|stop`, idempotent recovery, uncertain-mutation observation barriers and evidence-gated completion replace the old volatile whole-turn execution loop.
- Added closed server-owned `storage`, `building`, `mining` and `combat` workflow guidance without arbitrary file loading. Defaults allow 128 confirmed world actions, 192 model calls and three hours of active execution; the Numen build action accepts a bounded 256 KiB structure plan while other action/results remain smaller.
...
- The custom Citizens release jar is tracked in `overrides/mods/` and locked by SHA-256; the brain image builds from the public `zapeg-citizens` tag `v0.4.0`. The v0.4.0 jar and protocol-3 brain must be deployed together, and the Minecraft world ledger plus brain SQLite volume must be backed up/restored as one pair.
...
- Upgraded the mandatory client/server ZapeG Runtime to 0.2.0 and exact network protocol 2. The original target-private `echo_01` keeps wire ID 0; new allowlisted profiles add a gaze-withdrawing partial silhouette (`threshold_01`), a bounded 0.6-second local movement echo (`motion_echo_01`) and a screen-space light fault (`light_fault_01`). None creates a world entity, collision, loot or observer packet, and every effect expires or clears on player lifecycle changes.
- Added persistent OP-controlled Heraldor campaign phases and a fail-closed `/zapeg-lore director` bridge. Permission-level-2 players and authenticated RCON can inspect, pause/resume and monotonically advance `dormant → presence → servants → manifestation`, rehearse or trigger phase-gated Runtime profiles, and cancel the active scene. KubeJS only writes one short-lived world-bound request; the SQLite Director validates and records it before issuing an idempotent low-level `/zapegscene` command. Command blocks/functions are rejected, raw `/zapegscene` remains an effects-only override, and dormant/paused threshold audio is terminally suppressed rather than replayed later.
...
- Added `MertOnal`'s 5 km minecart quest, persistent tame/chest counters and a Turkish migration/test runbook. Group trophy items are claimable once per team. Enes's crossbow is now exact-name verified for `Thekingim` and reconciles an existing `ol_betsy` advancement on login.
- Corrected the identity registry: the car-fan Mert previously keyed as `MubarekAbi` is actually `MertOnal`; the other rare Mert returns to the unconfirmed `Mert` placeholder. Salih's exact login is `SalihKarahan`, not the previously recorded `SalihKarahans`, and Enes's exact login is `Thekingim`. Welcome/gift keys, quest ownership, rewards, offline UUID aliases and documentation now use the corrected identities.
- Added one-shot migrations so `MertOnal`, `SalihKarahan` and `Thekingim` receive their formerly missed named starter gifts if they already claimed the generic kit under the corrected login. Mert's additional objective is an OP-inspected habitable house. Added Emin Taha's OP-inspected public fountain plus separate owner-UUID dragon objectives for Emir and Salih; mounting an owned tamed Ice and Fire dragon also verifies the group milestone.
- Added four owner-tagged quest keepsakes: MertOnal's deed, Emin's fountain heart, Emir's dragon horn and Salih's dragon flute. KubeJS delivers each once, directly to the exact login, with owner/quest NBT instead of exposing it as a shared FTB Teams claim; a full inventory pauses and retries delivery instead of dropping the item.
- Removed the planned 64-rail objective, its coordinate counter/advancement criterion and the `MertOnal Ekspresi` quest keepsake. Mert's existing 5 km minecart and OP-inspected house objectives remain.
- Cached confirmed quest criteria per online UUID, short-circuited completed threshold/dragon/moon checks, stopped idle chest-score rewrites and reduced incomplete personal-reward advancement queries to five-second intervals. Manual guide buttons remain click-only and no world/entity scan was added.
...
- Replaced the shortcut-only Muhtar v1 experience with **Muhtar v2**, a static conversation-first mod consultant. He compares core ATM9 choices and ZapeG additions, explains prerequisites, first steps and common traps, then optionally opens the relevant landing quest. Every handoff closes the Easy NPC session, waits one tick and uses a permission-0 fixed-route `zapeg-guide` bridge; the former broad `ftbquests` execute-as-user allowlist is empty. The v1 layout is retained as a bridge-safe rollback, and fixed-UUID update/removal remains independently reversible.
- Added ten curated FTB Quest chapters for all player-facing ZapeG additions: Ice and Fire, Immersive Petroleum, Alex's Caves, Aquamirae, Mowzie's Mobs, Born in Chaos, Simply Swords/Better Combat, Immersive Vehicles/Eureka/Nifty Ships, Citizens/Numen and Incendium. Native mod advancements or exact item tasks prove measurable progress; explicit manual checks are limited to driving/sailing persistence, safe combat practice, citizen response and new-chunk exploration. Dependencies and operator-only infrastructure deliberately receive no filler pages.
