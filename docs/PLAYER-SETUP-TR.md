...
3. Oyunu ve CurseForge'ı kapat; `mods` içindeki tüm `cc-tweaked-1.20.1-forge-*.jar` ve `zapeg-citizens-forge-1.20.1-*.jar` dosyalarını sil. Buna eski `cc-tweaked-1.20.1-forge-1.113.1.jar` ile Citizens 0.2.0/0.2.1 jarları da dahildir.
...
7. Yamayı çıkarmadan önce `mods` içindeki **tüm** `cc-tweaked-1.20.1-forge-*.jar` ve `zapeg-citizens-forge-1.20.1-*.jar` dosyalarını sil. Özellikle `cc-tweaked-1.20.1-forge-1.113.1.jar` ile Citizens 0.2.0/0.2.1 jarları artık bulunmamalı.
...
9. Doğru kurulumda şu yapı görünür:
   ...
   Bu iki mod ailesi için yalnız `cc-tweaked-1.20.1-forge-1.116.1.jar` ve `zapeg-citizens-forge-1.20.1-0.3.0.jar` bulunmalı; diğer ATM9/ZapeG modlarını silme.
...
5. Zip'i çıkarmadan önce `mods` içindeki tüm `cc-tweaked-1.20.1-forge-*.jar` ve `zapeg-citizens-forge-1.20.1-*.jar` dosyalarını sil; buna eski `cc-tweaked-1.20.1-forge-1.113.1.jar` ile Citizens 0.2.0/0.2.1 jarları da dahildir.
6. Offline zip'inin içindekileri bu oyun klasörüne çıkar; üzerine yazmayı onayla.
7. `mods\iceandfire-....jar` dosyasının doğrudan profil altında olduğunu ve bu iki mod ailesi için yalnız `cc-tweaked-1.20.1-forge-1.116.1.jar` ile `zapeg-citizens-forge-1.20.1-0.3.0.jar` bulunduğunu kontrol et; diğer ATM9/ZapeG modlarını silme. Arada ikinci bir ZapeG klasörü olmamalı.
...
- `mods` içinde bu iki mod ailesinden birinin iki farklı sürümü varsa eski jar'ı kaldır. CC:Tweaked ailesinde yalnız `cc-tweaked-1.20.1-forge-1.116.1.jar`, Citizens ailesinde yalnız `zapeg-citizens-forge-1.20.1-0.3.0.jar` kalmalı; diğer ATM9/ZapeG modlarını silme. Eski CC 1.113.1 ve Citizens 0.2.0/0.2.1 silinmelidir.
