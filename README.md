...
| Faithful 32x | June 2026 | Modrinth version `VgTWEXF2`, **server-pushed** | Default HD resource pack, vanilla-faithful (everything recognizable). ~11 MB, served from the official Modrinth CDN via `resource-pack`/sha1 in compose — never enters the client patch or locks. Players may decline (`RESOURCE_PACK_ENFORCE=FALSE`). Credit: [faithfulpack.net](https://faithfulpack.net) |
...
3. **Phase 3 — custom layer + generated config pass, before the real world:** apply `scripts/apply-overrides.sh`, restart, and verify the three original ZapeG pages plus the ten added-mod guides for Ice and Fire, Immersive Petroleum, Alex's Caves, Aquamirae, Mowzie's Mobs, Born in Chaos, Simply Swords/Better Combat, vehicles/ships, Citizens and Incendium. Measurable tasks must use real item/advancement evidence; the few explicit manual checks cover only behavior the server cannot observe. Run the two-client checks in `docs/QUEST-VALIDATION-TR.md`. Place the tracked, stateless Muhtar v2 only after choosing his town-square coordinates, then run the non-OP dialogue and repeated-open matrix in `docs/MUHTAR-QUEST-GUIDE-TR.md`; deleting him never alters quest progress. Then run `scripts/iceandfire-config-check.sh`; edit the surfaced config so Ice and Fire silver ore generation is **off** and dragon griefing is low/none, snapshot, then restart. Doing this after pregen would leave duplicate silver in every generated chunk. Endgame policy stays level 1 (social); level-3 hooks remain dormant.
